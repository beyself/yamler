...
def create():
    ...
    if request.method=='POST' and g.user.id and form.validate():
        ...
        db_session.commit()
        if company.id:
            conn.execute(users.update().values({users.c.company_id: company.id}).where(users.c.id==g.user.id))
...
@mod.route('/get')
def get():
    rows = conn.execute(select([users.c.id, users.c.realname],users.c.company_id==g.company.id)).fetchall()
    data = []
    name = []
    for row in rows:
